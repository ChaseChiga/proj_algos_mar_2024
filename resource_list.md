# Resource list - built by you!

These links were added by you, the students, and Adrian!!  Disclaimer: no company is officially endorsing any of these resources or links.

## Python:
- [Jinja with Flask](https://flask.palletsprojects.com/en/2.3.x/templating/)

## MySQL:
- [MySQL practice](https://www.sql-practice.com/)
- [Visual of SQL joins](https://blog.codinghorror.com/a-visual-explanation-of-sql-joins/)
- [SQL Zoo](https://sqlzoo.net/wiki/SQL_Tutorial)
- [SQL CheatSheet](https://learnsql.com/blog/sql-basics-cheat-sheet/)

## JavaScript/MERN:
- [React documentation](https://react.dev/reference/react)
- [Mongoose model](https://mongoosejs.com/docs/api/model.html)
- [Mongoose queries](https://mongoosejs.com/docs/queries.html)
- [Mongoose middleware](https://mongoosejs.com/docs/middleware.html)
- [React-router-dom](https://reactrouter.com/en/main)
- [react-router browser router](https://reactrouter.com/en/main/router-components/browser-router)
- [useNavigate](https://reactrouter.com/en/main/hooks/use-navigate)
- [useParams](https://reactrouter.com/en/main/hooks/use-params)
- [Link](https://reactrouter.com/en/main/components/link)
- [node.js](https://nodejs.org/docs/latest/api/)
- [process - node.js](https://nodejs.org/docs/latest/api/process.html)
- [errors node.js](https://nodejs.org/docs/latest/api/errors.html)
- [nodes.js modules](https://nodejs.org/docs/latest/api/packages.html)
- [express- home](https://expressjs.com/)
- [express middleware](https://expressjs.com/en/resources/middleware.html)
- [express- routing](https://expressjs.com/en/starter/basic-routing.html)
- [adv routing](https://expressjs.com/en/guide/routing.html)
- [useContext](https://react.dev/reference/react/useContext)
- [useEffect](https://react.dev/reference/react/useEffect)
- [useReducer](https://react.dev/reference/react/useReducer)
- [useState](https://react.dev/reference/react/useState)
- [input object -react](https://react.dev/reference/react-dom/components/input)
- [form object- react](https://react.dev/reference/react-dom/components/form)
- [mongoDB docs](https://www.mongodb.com/docs/)
- [mongodb node.js driver](https://www.mongodb.com/docs/drivers/node/current/)
- [Vanilla JS Docs](https://developer.mozilla.org/en-US/docs/Web/javascript)

## Java:
- [Baeldung](https://www.baeldung.com/)
- [Baeldung full archive](https://www.baeldung.com/full_archive)
- [Java Basics - baeldung](https://www.baeldung.com/get-started-with-java-series)
- [spring boot- baeldung](https://www.baeldung.com/spring-boot)
- [baeldung maven](https://www.baeldung.com/maven-guide)
- [Spring Restful conventions](https://www.baeldung.com/rest-with-spring-series)
- [Baeldung- spring security](https://www.baeldung.com/security-spring)
- [Spring docs home](https://docs.spring.io/spring-framework/reference/index.html)
- [SPQL Spring Expression language](https://docs.spring.io/spring-framework/reference/core/expressions.html)
- [JPA - Spring](https://docs.spring.io/spring-framework/reference/data-access/orm/jpa.html)
- [JPA Query Methods](https://docs.spring.io/spring-data/jpa/docs/current-SNAPSHOT/reference/html/#jpa.query-methods)
- [@Query Annotation](https://docs.spring.io/spring-data/jpa/docs/current-SNAPSHOT/reference/html/#jpa.query-methods.at-query)
- [Spring Boot](https://spring.io/projects/spring-boot)
- [Spring repositories](https://docs.spring.io/spring-data/commons/docs/current/api/index.html)
- [JPQL query keywords](https://docs.spring.io/spring-data/jpa/reference/repositories/query-keywords-reference.html)
- [Maven Central Repository Search](https://search.maven.org/)

## Web (general):
- [HTTP documentation (including status codes) from Mozilla](https://developer.mozilla.org/en-US/docs/Web/HTTP)

## CSS:
- [Bootstrap cheat sheet](https://getbootstrap.com/docs/5.3/examples/cheatsheet/)
- [Another Bootstrap cheat sheet](https://bootstrap-cheatsheet.themeselection.com/)
- [CSS Resets](https://perishablepress.com/a-killer-collection-of-global-css-reset-styles/)
- [Flexbox guide](https://css-tricks.com/snippets/css/a-guide-to-flexbox/)
- [react-bootstrap](https://react-bootstrap.github.io/)
- [react-bootstrap-navbar](https://react-bootstrap.github.io/docs/components/navbar)
- [react-bootstrap-forms](https://react-bootstrap.github.io/docs/forms/overview)
- [react-bootstrap layout](https://react-bootstrap.github.io/docs/layout/grid)

## Algorithms:
- [Hacker Rank](https://www.hackerrank.com)
- [LeetCode](https://leetcode.com)
- [NeetCode](https://neetcode.io/)
- [Code Wars](https://www.codewars.com)
- [VisuAlgo](https://visualgo.net/en) - Visuals of data structures, sorts, etc.
- [CodinGame](https://www.codingame.com/start/)
- [Data structures and algorithms](https://techdevguide.withgoogle.com/paths/data-structures-and-algorithms/?no-filter=true)

## Videos from YouTube and other places:
- [Data structures and algorithms](https://www.youtube.com/watch?v=CBYHwZcbD-s&t=2s)
- [Videos about coding - with great explanations](https://www.youtube.com/@WebDevSimplified)
- [BroCode](https://www.youtube.com/@BroCodez)
- [Harvard CS50 course](https://www.youtube.com/watch?v=4oqjcKenCH8)

## General:
- [Python Tutor - visualize functions](https://pythontutor.com/)
- [MDN Docs](https://developer.mozilla.org/en-US/)
- [Validator](https://validator.w3.org/)
- [W3 schools](https://www.w3schools.com/)
- [Developer Roadmaps](https://roadmap.sh/)

## Git:
- [Git cheatsheet](https://education.github.com/git-cheat-sheet-education.pdf)
- [Another git cheatsheet](https://www.atlassian.com/git/tutorials/atlassian-git-cheatsheet)
- [.gitignore generator](https://www.toptal.com/developers/gitignore)
- [API list](https://apilist.fun/)

## APIs:
- [Faker API](https://fakerjs.dev/api/)
- [pokeApi](https://pokeapi.co/docs/v2)
- [starwars api](https://swapi.dev/documentation)
- [Public API list on GitHub](https://github.com/public-apis/public-apis)

## Other resources:
- [Cool slideshow and commands](https://codingdojo-onl-pt-versioncontrol.github.io/terminalGit/terminalCommands.html)
- [More bookmarked resources](https://hackmd.io/uv_lg3AdTvymhf5hf4tGQg?view#Terminal-Git-Github-Main-Notebook)